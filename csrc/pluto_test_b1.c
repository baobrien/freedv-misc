	...
	while (true) {
		...
		while (run) {
			...
			if (tx_burst_time == 0) {
				...
				memset(p_dat, 0, p_end - p_dat);
			} else if ( (tx_samp_count + p_samps) > tx_burst_time && tx_burst_n && !tx_redo) {
				int zr_len = tx_burst_time - tx_samp_count;
				int br_len = p_samps - zr_len;
				if (br_len > tx_burst_n) {
					br_len = tx_burst_n;
				}
				printf("tbt: %lld rbt:%lld ts:%lld n:%d\n", tx_burst_time, rx_samp_count, tx_samp_count, tx_burst_n);
				printf("zr_len %d br_len %d\n",zr_len,br_len);
				assert(zr_len >= 0);
				assert(br_len >= 0);
				memset(p_dat, 0, p_end - p_dat);
				p_dat += p_inc * zr_len;
				...
				float complex * buf;
				int nread_tx = 0;
				cbuffercf_read(out1_buffer, br_len, &buf, &nread_tx);
				cbuffercf_release(out1_buffer, nread_tx);
				for (; p_dat < p_end; p_dat += p_inc, tx_burst_ptr++) {
					// const int16_t i = ((int16_t*)p_dat)[0]; // Real (I)
					// const int16_t q = ((int16_t*)p_dat)[1]; // Imag (Q)
					((int16_t*)p_dat)[0] = (int16_t) (M_TO_R * crealf(*tx_burst_ptr));
					((int16_t*)p_dat)[1] = (int16_t) (M_TO_R * cimagf(*tx_burst_ptr));
					tx_burst_time++;
				}
				tx_burst_n -= br_len;
				if(tx_burst_n == 0){
